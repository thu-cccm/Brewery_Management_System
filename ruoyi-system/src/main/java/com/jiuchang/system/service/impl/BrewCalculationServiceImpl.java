...
public class BrewCalculationServiceImpl implements IBrewCalculationService
{
    ...
    public List<Map<String, Object>> calculateMaterial(Long templateId, Double targetOutput)
    {
        ...
        if (templateId == null || templateId <= 0) {
            throw new IllegalArgumentException("配方模板ID不能为空且必须大于0");
        }
        
        if (targetOutput == null || targetOutput <= 0) {
            throw new IllegalArgumentException("目标产量必须大于0");
        }
        
        if (targetOutput > 1000000) {
            throw new IllegalArgumentException("目标产量不能超过1000000L");
        }
    ...
    public Integer recommendCycle(Double temperature, Double humidity, String wineType)
    {
        if (temperature == null || temperature < -50 || temperature > 100) {
            throw new IllegalArgumentException("温度必须在-50℃到100℃之间");
        }
        
        if (humidity == null || humidity < 0 || humidity > 100) {
            throw new IllegalArgumentException("湿度必须在0%到100%之间");
        }
