...
CREATE TABLE brew_material (
  ...
  material_type         CHAR(1)         NOT NULL                   COMMENT '原料类型（1粮食类 2辅料类 3包装类）',
  ...
  stock_quantity        DECIMAL(12,4)   DEFAULT 0.0000             COMMENT '当前库存数量',
  stock_warning_value   DECIMAL(12,4)   DEFAULT 0.0000             COMMENT '库存预警阈值',
...
CREATE TABLE brew_wine_category (
  ...
  category_type         CHAR(1)         NOT NULL                   COMMENT '酒类型（1白酒 2啤酒 3黄酒 4果酒）',
...
CREATE TABLE brew_formula_template (
  ...
  base_output           DECIMAL(12,4)   NOT NULL                   COMMENT '基准产量（L）',
...
CREATE TABLE brew_formula_detail (
  ...
  material_ratio        DECIMAL(12,4)   NOT NULL                   COMMENT '配比系数（原料用量/基准产量）',
  base_quantity         DECIMAL(12,4)   NOT NULL                   COMMENT '基准用量',
...
CREATE TABLE brew_stock_record (
  ...
  material_id           BIGINT(20)      NOT NULL                   COMMENT '原料ID',
  change_type           CHAR(1)         NOT NULL                   COMMENT '变动类型（1入库 2出库 3盘点 4报损）',
  change_quantity       DECIMAL(12,4)   NOT NULL                   COMMENT '变动数量（正数入库，负数出库）',
  before_quantity       DECIMAL(12,4)   NOT NULL                   COMMENT '变动前库存',
  after_quantity        DECIMAL(12,4)   NOT NULL                   COMMENT '变动后库存',
  related_batch_no      VARCHAR(50)     DEFAULT NULL               COMMENT '关联批次号（入库时填供应商批次，出库时填生产批次）',
...
CREATE TABLE brew_production_plan (
  ...
  target_output         DECIMAL(12,4)   NOT NULL                   COMMENT '目标产量（L）',
  ...
  plan_status           CHAR(1)         DEFAULT '1'                COMMENT '计划状态（1待审核 2已审核 3生产中 4已完成 5已取消）',
...
CREATE TABLE brew_production_batch (
  ...
  category_id           BIGINT(20)      NOT NULL                   COMMENT '酒品种类ID',
  template_id           BIGINT(20)      NOT NULL                   COMMENT '配方模板ID',
  actual_output         DECIMAL(12,4)   DEFAULT 0.0000             COMMENT '实际产量（L）',
  ...
  responsible_person    VARCHAR(64)     DEFAULT NULL               COMMENT '负责人',
  batch_status          CHAR(1)         DEFAULT '1'                COMMENT '批次状态（1备料中 2发酵中 3蒸馏中 4质检中 5已完成 6异常）',
  quality_status        CHAR(1)         DEFAULT '0'                COMMENT '质检状态（0未检 1合格 2不合格）',
  remark                VARCHAR(500)    DEFAULT NULL               COMMENT '备注',
  create_by             VARCHAR(64)     DEFAULT ''                 COMMENT '创建者',
  create_time           DATETIME                                   COMMENT '创建时间',
  update_by             VARCHAR(64)     DEFAULT ''                 COMMENT '更新者',
...
CREATE TABLE brew_batch_material (
  ...
  material_id           BIGINT(20)      NOT NULL                   COMMENT '原料ID',
  material_batch_no     VARCHAR(50)     DEFAULT NULL               COMMENT '原料批次号（供应商提供，可手工录入）',
  planned_quantity      DECIMAL(12,4)   NOT NULL                   COMMENT '计划用量',
...
CREATE TABLE brew_production_process (
  ...
  batch_no              VARCHAR(50)     NOT NULL                   COMMENT '批次号',
  process_stage         CHAR(1)         NOT NULL                   COMMENT '工序阶段（1备料 2发酵 3蒸馏 4灌装）',
...
CREATE TABLE brew_quality_record (
  ...
  -- 综合判定
  quality_result        CHAR(1)         NOT NULL                   COMMENT '质检结果（1合格 2不合格 3待复检）',
...
CREATE TABLE brew_quality_report (
  ...
  report_status         CHAR(1)         DEFAULT '1'                COMMENT '报告状态（1草稿 2已发布 3已归档）',
  approve_status        CHAR(1)         DEFAULT '0'                COMMENT '审批状态（0待审批 1已通过 2已驳回）',
...
CREATE TABLE brew_cost_item (
  ...
  item_type             CHAR(1)         NOT NULL                   COMMENT '成本类型（1原料成本 2能耗成本 3人工成本 4其他）',
  calculation_method    CHAR(1)         DEFAULT '1'                COMMENT '计算方式（1固定值 2按产量 3按工时）',
...
-- 4.2 批次成本明细表（修复版：关联成本项）
DROP TABLE IF EXISTS brew_batch_cost;
CREATE TABLE brew_batch_cost (
  cost_id               BIGINT(20)      NOT NULL AUTO_INCREMENT    COMMENT '成本ID',
  batch_no              VARCHAR(50)     NOT NULL                   COMMENT '批次号',
  cost_item_id          BIGINT(20)      NOT NULL                   COMMENT '成本项ID',
  actual_cost           DECIMAL(10,2)   NOT NULL                   COMMENT '实际成本金额（元）',
  cost_quantity         DECIMAL(12,4)   DEFAULT NULL               COMMENT '成本数量（如工时、用电量）',
  ...
  PRIMARY KEY (cost_id),
  KEY idx_batch_no (batch_no),
  KEY idx_cost_item_id (cost_item_id)
...
CREATE TABLE brew_cost_statistics (
  ...
  stat_type             CHAR(1)         NOT NULL                   COMMENT '统计类型（1月度 2季度 3年度）',
  total_output          DECIMAL(12,4)   DEFAULT 0.0000             COMMENT '总产量（L）',
  ...
  cost_trend            CHAR(1)         DEFAULT NULL               COMMENT '成本趋势（1上升 2下降 3持平）',
  create_by             VARCHAR(64)     DEFAULT ''                 COMMENT '创建者',
  create_time           DATETIME                                   COMMENT '创建时间',
  update_by             VARCHAR(64)     DEFAULT ''                 COMMENT '更新者',
  update_time           DATETIME                                   COMMENT '更新时间',
...
-- 删除可能存在的旧字典
DELETE FROM sys_dict_type WHERE dict_id >= 100 AND dict_id <= 110;
DELETE FROM sys_dict_data WHERE dict_code >= 100 AND dict_code <= 200;
...
INSERT INTO sys_dict_type VALUES (102, '生产计划状态', 'brew_plan_status', '0', 'admin', SYSDATE(), '', NULL, '生产计划状态');
INSERT INTO sys_dict_type VALUES (103, '生产批次状态', 'brew_batch_status', '0', 'admin', SYSDATE(), '', NULL, '生产批次状态');
INSERT INTO sys_dict_type VALUES (104, '质检结果', 'brew_quality_result', '0', 'admin', SYSDATE(), '', NULL, '质检结果类型');
INSERT INTO sys_dict_type VALUES (105, '成本类型', 'brew_cost_type', '0', 'admin', SYSDATE(), '', NULL, '成本类型列表');
INSERT INTO sys_dict_type VALUES (106, '库存变动类型', 'brew_stock_change_type', '0', 'admin', SYSDATE(), '', NULL, '库存变动类型');
INSERT INTO sys_dict_type VALUES (107, '工序阶段', 'brew_process_stage', '0', 'admin', SYSDATE(), '', NULL, '生产工序阶段');
INSERT INTO sys_dict_type VALUES (108, '计算方式', 'brew_calculation_method', '0', 'admin', SYSDATE(), '', NULL, '成本计算方式');
...
-- 5.4 生产计划状态字典
INSERT INTO sys_dict_data VALUES (120, 1, '待审核', '1', 'brew_plan_status', '', 'info', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (121, 2, '已审核', '2', 'brew_plan_status', '', 'primary', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (122, 3, '生产中', '3', 'brew_plan_status', '', 'warning', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (123, 4, '已完成', '4', 'brew_plan_status', '', 'success', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (124, 5, '已取消', '5', 'brew_plan_status', '', 'danger', 'N', '0', 'admin', SYSDATE(), '', NULL, '');

-- 5.5 生产批次状态字典
INSERT INTO sys_dict_data VALUES (130, 1, '备料中', '1', 'brew_batch_status', '', 'info', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (131, 2, '发酵中', '2', 'brew_batch_status', '', 'primary', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (132, 3, '蒸馏中', '3', 'brew_batch_status', '', 'warning', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (133, 4, '质检中', '4', 'brew_batch_status', '', 'default', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (134, 5, '已完成', '5', 'brew_batch_status', '', 'success', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (135, 6, '异常', '6', 'brew_batch_status', '', 'danger', 'N', '0', 'admin', SYSDATE(), '', NULL, '');

-- 5.6 质检结果字典
INSERT INTO sys_dict_data VALUES (140, 1, '合格', '1', 'brew_quality_result', '', 'success', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (141, 2, '不合格', '2', 'brew_quality_result', '', 'danger', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (142, 3, '待复检', '3', 'brew_quality_result', '', 'warning', 'N', '0', 'admin', SYSDATE(), '', NULL, '');

-- 5.7 成本类型字典
INSERT INTO sys_dict_data VALUES (150, 1, '原料成本', '1', 'brew_cost_type', '', 'primary', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (151, 2, '能耗成本', '2', 'brew_cost_type', '', 'warning', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (152, 3, '人工成本', '3', 'brew_cost_type', '', 'success', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (153, 4, '其他成本', '4', 'brew_cost_type', '', 'info', 'N', '0', 'admin', SYSDATE(), '', NULL, '');

-- 5.8 库存变动类型字典
INSERT INTO sys_dict_data VALUES (160, 1, '入库', '1', 'brew_stock_change_type', '', 'success', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (161, 2, '出库', '2', 'brew_stock_change_type', '', 'warning', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (162, 3, '盘点', '3', 'brew_stock_change_type', '', 'primary', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (163, 4, '报损', '4', 'brew_stock_change_type', '', 'danger', 'N', '0', 'admin', SYSDATE(), '', NULL, '');

-- 5.9 工序阶段字典
INSERT INTO sys_dict_data VALUES (170, 1, '备料', '1', 'brew_process_stage', '', 'info', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (171, 2, '发酵', '2', 'brew_process_stage', '', 'primary', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (172, 3, '蒸馏', '3', 'brew_process_stage', '', 'warning', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (173, 4, '灌装', '4', 'brew_process_stage', '', 'success', 'N', '0', 'admin', SYSDATE(), '', NULL, '');

-- 5.10 计算方式字典
INSERT INTO sys_dict_data VALUES (180, 1, '固定值', '1', 'brew_calculation_method', '', 'primary', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (181, 2, '按产量', '2', 'brew_calculation_method', '', 'success', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
INSERT INTO sys_dict_data VALUES (182, 3, '按工时', '3', 'brew_calculation_method', '', 'warning', 'N', '0', 'admin', SYSDATE(), '', NULL, '');
...
-- 初始化原料数据
INSERT INTO brew_material VALUES (1, 'MAT001', '高粱', '1', 'kg', '东北红高粱', 5.50, '东北粮食供应商', '13800138000', 5000.0000, 1000.0000, '0', '0', '优质酿酒原料', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_material VALUES (2, 'MAT002', '小麦', '1', 'kg', '优质小麦', 4.20, '东北粮食供应商', '13800138000', 3000.0000, 800.0000, '0', '0', '', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_material VALUES (3, 'MAT003', '大米', '1', 'kg', '优质大米', 6.00, '南方粮食供应商', '13900139000', 2000.0000, 500.0000, '0', '0', '', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_material VALUES (4, 'MAT004', '酒曲', '2', 'kg', '大曲', 25.00, '酒曲厂', '13700137000', 200.0000, 50.0000, '0', '0', '发酵必需品', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_material VALUES (5, 'MAT005', '山泉水', '2', 'L', '天然山泉水', 0.50, '本地水源', '', 50000.0000, 10000.0000, '0', '0', '', 'admin', SYSDATE(), '', NULL, '0');

-- 初始化酒品种类
INSERT INTO brew_wine_category VALUES (1, 'WINE001', '浓香型白酒', '1', 52.00, 56.00, 60, 90, 28.00, 70.00, '传统浓香型白酒', '0', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_wine_category VALUES (2, 'WINE002', '清香型白酒', '1', 50.00, 53.00, 45, 60, 20.00, 65.00, '清香型白酒', '0', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_wine_category VALUES (3, 'WINE003', '精酿啤酒', '2', 4.00, 6.00, 7, 14, 12.00, 75.00, '精酿啤酒', '0', 'admin', SYSDATE(), '', NULL, '0');

-- 初始化配方模板
INSERT INTO brew_formula_template VALUES (1, 'TPL001', '浓香型白酒标准配方', 1, 1000.0000, 53.00, '传统浓香型白酒配方', '1', '0', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_formula_template VALUES (2, 'TPL002', '清香型白酒标准配方', 2, 1000.0000, 52.00, '清香型白酒配方', '1', '0', 'admin', SYSDATE(), '', NULL, '0');

-- 初始化配方明细
INSERT INTO brew_formula_detail VALUES (1, 1, 1, 0.5000, 500.0000, '1', 1, '主料', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_formula_detail VALUES (2, 1, 2, 0.3000, 300.0000, '1', 2, '主料', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_formula_detail VALUES (3, 1, 3, 0.2000, 200.0000, '0', 3, '辅料', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_formula_detail VALUES (4, 1, 4, 0.0200, 20.0000, '0', 4, '酒曲', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_formula_detail VALUES (5, 1, 5, 3.0000, 3000.0000, '0', 5, '水', 'admin', SYSDATE(), '', NULL, '0');

-- 初始化成本项
INSERT INTO brew_cost_item VALUES (1, 'COST001', '原料采购成本', '1', '1', 0.00, '0', '按实际采购价计算', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_cost_item VALUES (2, 'COST002', '电费', '2', '2', 0.80, '0', '每升产量0.8元', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_cost_item VALUES (3, 'COST003', '燃气费', '2', '2', 1.20, '0', '每升产量1.2元', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_cost_item VALUES (4, 'COST004', '人工工资', '3', '3', 150.00, '0', '每工时150元', 'admin', SYSDATE(), '', NULL, '0');

-- 初始化质检标准
INSERT INTO brew_quality_standard VALUES (1, 1, '浓香型白酒国标GB/T 10781.1', 52.00, 56.00, 1.00, '无色透明', '窖香浓郁', '绵甜爽净', '{"total_acid":"≥0.4g/L","total_ester":"≥2.0g/L"}', '1', '0', 'admin', SYSDATE(), '', NULL, '0');
INSERT INTO brew_quality_standard VALUES (2, 2, '清香型白酒国标GB/T 10781.2', 50.00, 53.00, 1.00, '无色透明', '清香纯正', '清爽干净', '{"total_acid":"≥0.3g/L","total_ester":"≥1.5g/L"}', '1', '0', 'admin', SYSDATE(), '', NULL, '0');
